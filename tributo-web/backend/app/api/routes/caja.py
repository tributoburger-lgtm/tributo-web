...
def turno_activo(db: Session = Depends(get_db), user=Depends(get_current_user)):
...
def abrir_turno(data: dict, db: Session = Depends(get_db), user=Depends(get_current_user)):
    existente = db.query(TurnoCaja).filter(
        TurnoCaja.usuario_id == user.id, TurnoCaja.estado == "ABIERTO"
    ).first()
    if existente:
...
def cerrar_turno(turno_id: int, data: dict, db: Session = Depends(get_db), user=Depends(get_current_user)):
...
@router.post("/egreso")
def registrar_egreso(data: dict, db: Session = Depends(get_db), user=Depends(get_current_user)):
    e = EgresoCaja(
        turno_id=data.get("turno_id"),
        usuario_id=user.id,
        categoria=data.get("categoria", "GASTO"),
...
@router.post("/ingreso")
def registrar_ingreso(data: dict, db: Session = Depends(get_db), user=Depends(get_current_user)):
    i = IngresoCaja(
        turno_id=data.get("turno_id"),
        usuario_id=user.id,
...
@router.get("/egresos")
def listar_egresos(desde: str = None, hasta: str = None, db: Session = Depends(get_db), _=Depends(get_current_user)):
    q = db.query(EgresoCaja).order_by(EgresoCaja.fecha.desc())
